...
// Print out the first name, last name for each president in the array of objects
// presidents.map(x => {console.log(x.first,x.last)})

// Create a new array (fullName) with each cell containing a first name, last name of the corresponding president object in the presidents array
let fullName = presidents.map(president => `${president.first} ${president.last}`);

// Same as above, with the function fully defined
let fullName = presidents.map(function(president) {
